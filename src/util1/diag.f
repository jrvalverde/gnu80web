...
C  COMMAND LINE: "ftangle -ybs15000 diag"
...
*line 23 "diag.web"
     ...
     &s,gamma,gsqrt,one,ord,p,pp,ppbr
      double precision ppbs,pt5,q,qj,r1,r12,r2,rhosq,s,sgn,shift,sina,si
     &na2,sqrts,sum,temp,tenm18,two,u,VEC
      double precision W,wj,wtaw,zero
      integer i,i1,iposv,itemp,ivpos,j,k,l,lv,m,MAXBAS,MAXDIM,n,n1,n2,NN
     &,np,npas,nr,nrr
      integer nt,nv
      parameter(MAXBAS=150)
      dimension A(MAXDIM,MAXDIM),VEC(MAXDIM,MAXDIM),EIG(MAXDIM),W(MAXDIM
     &)
      dimension gamma(MAXBAS),beta(MAXBAS),betasq(MAXBAS),p(MAXBAS),q(MA
     &XBAS),ord(MAXBAS),iposv(MAXBAS),ivpos(MAXBAS)
      equivalence(p(1),q(1),ivpos(1),beta(1)),(iposv(1),gamma(1)),(ord(1
     &),betasq(1))
